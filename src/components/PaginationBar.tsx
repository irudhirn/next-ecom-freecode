...
export default function PaginationBar({currentPage, totalPages}: PaginationBarProps) {
  ...
  for (let page = minPage; page <= maxPage; page++) {
    numberedPageItems.push(<Link href={`?page=${page}`} key={page} className={`join-item btn ${currentPage === page ? "btn-active pointer-events-none" : ""}`}>{page}</Link>);
  ...
  return (
    <>
      ...
      <div className="join block sm:hidden">
        ...
        {currentPage < totalPages && <Link href={"?page=" + (currentPage - 1)} className="join-item btn">»</Link>}
      </div>
    </>
  )
}
